...
import { GOOGLE_ANALYTICS_SCOPES } from "./scopes";
import open from "open";
import { logger } from "../utils/logger";

const TOKENS_DIR = path.join(os.homedir(), ".google-analytics-mcp");
...
export async function ensureValidToken(): Promise<void> {
  ...
  if (!client.credentials.refresh_token) {
    throw new Error(
      'No refresh token available. Please run "mcp-google-analytics auth" to authenticate.'
    );
  }
  ...
  if (expiryDate && expiryDate <= Date.now() + 5 * 60 * 1000) {
    try {
      ...
      throw new Error(
        'Token refresh failed. Please run "mcp-google-analytics auth" to re-authenticate.'
      );
    }
  }
}
