/**
 * Command-line interface handling for Google Analytics MCP Server
...
Quick Start (NPX - No Installation Required):
  ...
  2. Run: npx mcp-google-analytics auth
  3. Follow the authentication flow
  4. Run: npx mcp-google-analytics init
  5. Restart Claude Desktop

Quick Start (Global Installation):
  1. Install: npm install -g mcp-google-analytics
  2. Setup: mcp-google-analytics auth
  3. Check status: mcp-google-analytics status
  4. Configure: mcp-google-analytics init
  5. Restart Claude Desktop
`);
}
...
function showVersion(): void {
  ...
  console.log(`mcp-google-analytics v${version}`);
}

async function handleAuth(args: string[]): Promise<void> {
  console.log("🔐 Google Analytics MCP Authentication\n");
...
`);
  ...
  if (!force && hasValidTokens()) {
    ...
    console.log("\nTo re-authenticate, run: mcp-google-analytics auth --force");
    return;
  }
    ...
    rl.question("Authorization code: ", (code) => {
      ...
      void (async () => {
        try {
          ...
          console.log("\nYou can now use the Google Analytics MCP server.");
        } catch (error) {
          console.error(
...
function showStatus(): void {
  ...
  if (fs.existsSync(tokensPath)) {
    ...
    console.log('💡 Run "mcp-google-analytics auth" to authenticate');
  }
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (config?.mcpServers?.["google-analytics-mcp"]) {
        console.log("✅ google-analytics-mcp server is configured");
      } else {
        console.log("⚠️  google-analytics-mcp server not configured");
        console.log('💡 Run "mcp-google-analytics init" to configure');
      }
    } else {
      console.log("❌ Claude Desktop config file does not exist");
      console.log('💡 Run "mcp-google-analytics init" to create configuration');
    }
  } catch {
...
function interactiveSetup(_args: string[]): void {
  console.log("🚀 Setting up google-analytics-mcp for Claude Desktop...\n");
...
Next steps:
1. Run: mcp-google-analytics auth (if not already authenticated)
2. Restart Claude Desktop
3. Start a new conversation  
4. You should now have access to Google Analytics tools

The following tools will be available:
- Query Analytics reports and realtime data
- Manage properties and accounts
- Configure data filters
- Get custom dimensions and metrics
- Batch operations
- And more!

💡 Try asking Claude: "Get my Analytics account summaries" or "Run a report for property X"
`);
  } else {
